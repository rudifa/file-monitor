...
QString getLocalesDirectory(QApplication const& app)
{
    ...
    if (file_info.exists()) return path;

// If installed on unix, the locale files are in the PKGDATADIR as set by
// deployment.pri.
#ifdef PKGDATADIR
...
QString locale::getSystemTranslationUri(QApplication const& app)
{
    ...
    if (file_info.exists()) return tranlation_uri;
...
bool settings::loadSettingsFromFile(QString const& file_uri)
{
    ...
    if (temp_keys.isEmpty()) return false;
...
double math::rampUp(double input, double input_min, double input_max,
                    double output_min, double output_max)
{
    // We start the min a little low to try to get us very close to the
    // output_min.
    double const logistic_function_input_min = -10;
    // Stop half way through the logistic curve so that we only include the slow
    // start and steady ramp up.
    double const logistic_function_input_max = 0;

    double scaled_input = scaleValueBetweenRanges(input, input_min, input_max,
                                                  logistic_function_input_min,
                                                  logistic_function_input_max);

    double logistic_function_output = logisticFunction(scaled_input);

    double const logistic_function_output_min = 0;
    // We are only moving half-way through the logistic function, so the output
    // is .5 rather than 1.0.
    double const logistic_function_output_max = .5;

    double scaled_logistic_function_output = scaleValueBetweenRanges(
        logistic_function_output, logistic_function_output_min,
        logistic_function_output_max, output_min, output_max);
    return scaled_logistic_function_output;
}

double math::inverseRampUp(double input, double input_min, double input_max,
                           double output_min, double output_max)
{
    // We start the min a little low to try to get us very close to the
    // output_min.
    ...
    double scaled_input = scaleValueBetweenRanges(input, input_min, input_max,
                                                  logit_function_input_min,
                                                  logit_function_input_max);
    ...
    double scaled_logit_function_output = scaleValueBetweenRanges(
        logit_function_output, logit_function_output_min,
        logit_function_output_max, output_min, output_max);
