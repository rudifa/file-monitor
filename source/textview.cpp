...
TextView::TextView(Ui::MainWindow const &ui, QWidget *parent)
    : View(ViewScale(ViewScale::Linear, 1, 100, 12), parent),
      ...
      indent_xml(false),
      scroll_to_bottom_on_change(false)
{
}
...
bool TextView::load(QString const &file_uri, bool is_reload)
{
    ...
    connect(text_edit, SIGNAL(signalScaleChanged(double)),
            SLOT(slotScaleChanged(double)));

    // Only scroll bottom if we are reloading non-xml content.
    if (scroll_to_bottom_on_change && is_reload &&
        !xml::isXML(unformatted_content))
        QTimer::singleShot(0, this, SLOT(slotScrollToBottom()));

    return true;
}

void TextView::setScale(double scale) { text_edit->setScale(scale); }
...
{
    auto word_wrap_mode =
        (word_wrap ? QTextOption::WordWrap : QTextOption::NoWrap);
...
void TextView::selectAll() { text_edit->selectAll(); }

void TextView::copy() { text_edit->copy(); }

void TextView::slotFindNext(QString const &text, bool case_sensitive)
{
    QTextDocument::FindFlags find_flags;

    if (case_sensitive) find_flags |= QTextDocument::FindCaseSensitively;
    ...
    if (!success)
    {
        // This addresses a bug in which find fails when the text to be found
        // exists below a currently selected section.
        if (text_edit->toPlainText().contains(
                text, case_sensitive ? Qt::CaseSensitive : Qt::CaseInsensitive))
        {
            // If nothing is selected, then this is a strange error case we have
            // to figure out.
            assert(!text_edit->textCursor().selectedText().isEmpty());

            // Remove selection, find again, and bail if we found another
            // (otherwise, fall through to "Find First" below).
            text_edit->textCursor().clearSelection();
            if (text_edit->find(text, find_flags)) return;
        }

        // Wrap around by moving to the first instance of the text in the
        // document.
...
void TextView::slotFindPrevious(QString const &text, bool case_sensitive)
{
    ...
    if (!success)
    {
        // NOTE: If no text is selected, find previous fails.  We must find
        // forward first.
        if (text_edit->textCursor().selectedText().isEmpty())
            text_edit->find(text);

        // Wrap around by moving to the last instance of the text in the
        // document.
...
{
    text_edit->verticalScrollBar()->setSliderPosition(
        text_edit->verticalScrollBar()->maximumHeight());
}
