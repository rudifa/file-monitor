...
HtmlView::HtmlView(Ui::MainWindow const &ui, QWidget *parent)
    : View(ViewScale(ViewScale::Linear, .25, 5, 1), parent),
      web_view(new CustomWebView(ui, view_scale, parent))
{
}
...
bool HtmlView::load(QString const &file_uri, bool)
{
    ...
    connect(web_view, SIGNAL(signalScaleChanged(double)),
            SLOT(slotScaleChanged(double)));

    return true;
}

void HtmlView::setScale(double scale) { web_view->setScale(scale); }
...
void HtmlView::slotSetScroll()
{
    web_view->page()->runJavaScript(
        "(function() { "
        "   return {"
        "       maxH: document.documentElement.scrollWidth - "
        "document.documentElement.clientWidth,"
        "       maxV: document.documentElement.scrollHeight - "
        ...
        [this](const QVariant &result)
        {
            ...
            if (max_scroll_horizontal >= scroll_dimensions.x() &&
                max_scroll_vertical >= scroll_dimensions.y())
            {
                QString script = QString("window.scrollTo(%1, %2);")
                                     .arg(scroll_dimensions.x())
                                     .arg(scroll_dimensions.y());
...
{
    // This function now needs to be asynchronous, so we'll return a default
    // value You might want to redesign this to use a callback or signal
    return QPoint(0, 0);
}
...
void HtmlView::copy() { web_view->page()->triggerAction(QWebEnginePage::Copy); }
...
void HtmlView::slotFindNext(QString const &text, bool case_sensitive)
{
    ...
    if (case_sensitive) find_flags |= QWebEnginePage::FindCaseSensitively;
