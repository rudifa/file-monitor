...
MainWindow::MainWindow(QWidget* parent) : QMainWindow(parent)
{
    ...
    connect(ui.action_session_save_as, SIGNAL(triggered()),
            SLOT(slotSaveSessionAs()));
    connect(ui.action_session_open, SIGNAL(triggered()),
            SLOT(slotOpenSession()));
    connect(ui.action_about_file_monitor, SIGNAL(triggered()),
            SLOT(slotAboutFileMonitor()));
...
// Add this kind of code to every widget with translatable strings when we want
// the app
...
void MainWindow::slotSaveSessionAs()
{
    ...
    QString file_uri = QFileDialog::getSaveFileName(this, tr("Save Session"),
                                                    last_saved_session_uri,
                                                    tr("Session (*.ses)"));
...
void MainWindow::slotAboutFileMonitor()
{
    ...
    auto about_content =
        QString(
            "<h3>%1</h3><p>%2</p><p>%3</p><p>%4</p><p>%5</p><p>%6%7</p><hr "
            "/><p>%8</p><p>%9</p>")
            .arg(tr("File Monitor - ") + file_monitor_version)
            .arg(tr("File Monitor is a cross-platform file viewer that "
                    "automatically redraws files when they change on disk."))
            .arg(tr("You can edit an HTML document in a simple text editor and "
                    "watch it update in real-time using File Monitor."))
            .arg(tr("If you are keeping track of 10 different log files, File "
                    "Monitor will automatically switch to the most recently "
                    "modified file and take you to those changes."))
            .arg(tr("Currently supported file types are text, html, svg, and "
                    "raster image file formats (jpg, gif, png, bmp, etc)."))
            .arg(tr("To learn more about File Monitor or to contribute, please "
                    "visit our project page "))
            .arg("<a href=\"" + file_monitor_project_url + "\">" +
                 file_monitor_project_url + "</a>")
            .arg(tr("Copyright 2010-2012 Mark Turney.  All rights reserved."))
            .arg(tr("This program is provided AS IS with NO WARRANTY OF ANY "
                    "KIND, INCLUDING THE WARRANTY OF DESIGN, MERCHANTIBILITY "
...
void MainWindow::loadSettings()
{
    ...
    ui.action_close_deleted_files->setChecked(
        settings.value("closeDeletedFiles", false).toBool());
    ui.action_transparent_background->setChecked(
        settings.value("transparent_background", true).toBool());
    ui.action_word_wrap->setChecked(settings.value("word_wrap", true).toBool());
    ui.action_indent_xml->setChecked(
        settings.value("indentXML", true).toBool());
    ui.action_scroll_to_bottom->setChecked(
        settings.value("scrollToBottom", false).toBool());
...
void MainWindow::saveSettings()
{
    ...
    settings.setValue("close_deleted_files",
                      ui.action_close_deleted_files->isChecked());
    settings.setValue("transparent_background",
    ...
    settings.setValue("scrollToBottom",
                      ui.action_scroll_to_bottom->isChecked());
