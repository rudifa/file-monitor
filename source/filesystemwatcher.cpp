...
void FileSystemWatcher::slotFileChanged(QString file_uri)
{
    ...
    if (!file_info.exists())
    {
        if (deleted_files.empty()) deleted_files_timer->start(500);
...
{
    // If any of the deleted files have been restored, start watching them
    // again.
    for (std::size_t i = 0; i < deleted_files.size(); ++i)
    {
        QString file_uri = deleted_files[i];
        file_info->setFile(file_uri);
        if (file_info->exists()) addPath(file_uri);
    }

    file::clearUrisWithFileOnDisk(deleted_files);

    if (deleted_files.empty()) deleted_files_timer->stop();
}
